...
public class Challenge {
    ...
    public static void main(String[] args) {
        ...
        ArrayList<char[][]> solutions = getAllQs(8);

        System.out.println(solutions.size());

        for (int i = 0; i < solutions.size(); i++) {
            System.out.println("\n\nSolution " + (i + 1));
            if (qsAreSafe(solutions.get(i)))
    ...
    public static boolean qsAreSafe(char[][] board) {
        char[] validChars = { 'q', '.' };
        assert (board != null) && (board.length > 0)
               && isSquare(board) && onlyContains(board, validChars)
            : "Violation of precondition: qsAreSafe";

        return true;
    }

    public static ArrayList<char[][]> getAllQs(int size) {
        ArrayList<char[][]> solutions = new ArrayList<char[][]>();
        char[][] board = blankBoard(size);
        solveAllNQs(board, 0, solutions);
        return solutions;
    }

    public static void solveAllNQs(char[][] board, int col, ArrayList<char[][]> solutions) {
        ...
        if (col == board.length) {
            ...
            for (int row = 0; row < board.length; row++) {
                ...
                if (qsAreSafe(board))
                    ...
                    solveAllNQs(board, col + 1, solutions);
                board[row][col] = '.';
            }
        }
    }
    ...
    public static void solveTheChallenge(int n) {
        char[][] board = blankBoard(n);
        //start in column 0
        boolean solved = canSolve(board, 0);
        if (solved) {
            System.out.println("Solved the " + n + " q problem.");
            printBoard(board);
        } else
            System.out.println("Can't solve the " + n + " q problem.");
    }
